function [Results,MPC]=Runlinopf3CrossphaseVUFPareto(SN)
%% add library
addpath('C:\Users\akarimi8\OneDrive - Georgia Institute of Technology\OPF\Simulation\Libraries\Library Case Modification')
addpath('C:\Users\akarimi8\OneDrive - Georgia Institute of Technology\OPF\Simulation\Libraries\graph_cal')
...
cvx_begin
  ...
  variables va(n) teta(n) vb(n) tetb(n) vc(n) tetc(n)
   ...
   minimize((1-w)*V2ap(va,teta,vb,tetb,vc,tetc)+...
      w*(...
...
%  3ev
  for j=K3ev
        P3eva(j)+P3evb(j)+P3evc(j)==MPC.Device3EV(j,2)/MPC.baseMVA
        Cap3eva=linearcap(P3eva(j),Q3eva(j),acc);
        Cap3eva*MPC.baseMVA<=MPC.Device3EV(j,3)/3
        Cap3evb=linearcap(P3evb(j),Q3evb(j),acc);
        Cap3evb*MPC.baseMVA<=MPC.Device3EV(j,3)/3
        Cap3evc=linearcap(P3evc(j),Q3evc(j),acc);
        Cap3evc*MPC.baseMVA<=MPC.Device3EV(j,3)/3
  end
  ...
  for j=Keva
        ...
        Capeva=linearcap(Peva(j),Qeva(j),acc);
        Capeva*MPC.baseMVA<=MPC.a.DeviceEV(j,3)      
  end
  for j=setdiff(MPC.All,Keva)
   ...
   for j=Kevb
        ...
        Capevb=linearcap(Pevb(j),Qevb(j),acc);
        Capevb*MPC.baseMVA<=MPC.b.DeviceEV(j,3)
   end
  for j=setdiff(MPC.All,Kevb)
   ...
   for j=Kevc
        ...
        Capevc=linearcap(Pevc(j),Qevc(j),acc);
        Capevc*MPC.baseMVA<=MPC.c.DeviceEV(j,3)
   end
  ...
  % 3pv
  for j=K3pv
        P3pva(j)+P3pvb(j)+P3pvc(j)<=MPC.Device3PV(j,2)/MPC.baseMVA
        P3pva(j)+P3pvb(j)+P3pvc(j)>=0
        Cap3pva=linearcap(P3pva(j),Q3pva(j),acc);
        Cap3pva*MPC.baseMVA<=MPC.Device3PV(j,3)/3
        Cap3pvb=linearcap(P3pvb(j),Q3pvb(j),acc);
        Cap3pvb*MPC.baseMVA<=MPC.Device3PV(j,3)/3
        Cap3pvc=linearcap(P3pvc(j),Q3pvc(j),acc);
        Cap3pvc*MPC.baseMVA<=MPC.Device3PV(j,3)/3
  end
...
%  pva
  for j=Kpva
        ...
        Ppva(j)>=0
        Cappva=linearcap(Ppva(j),Qpva(j),acc);
        Cappva*MPC.baseMVA<=MPC.a.DevicePV(j,3)
  end
  ...
  for j=Kpvb
        ...
        Cappvb=linearcap(Ppvb(j),Qpvb(j),acc);
        Cappvb*MPC.baseMVA<=MPC.b.DevicePV(j,3) 
  end
  ...
  for j=Kpvc
        ...
        Ppvc(j)>=0
        Cappvc=linearcap(Ppvc(j),Qpvc(j),acc);
        Cappvc*MPC.baseMVA<=MPC.c.DevicePV(j,3)  
  end
...
results3.Costtotal(1,1)=+Pgena'.^2*MPC.a.gencost(:,5)*(MPC.baseMVA)^2+Pgena'*MPC.a.gencost(:,6)*(MPC.baseMVA)+sum(MPC.a.gencost(:,7))...
 +Pgenb'.^2*MPC.b.gencost(:,5)*(MPC.baseMVA)^2+Pgenb'*MPC.b.gencost(:,6)*(MPC.baseMVA)+sum(MPC.b.gencost(:,7))...
 +Pgenc'.^2*MPC.c.gencost(:,5)*(MPC.baseMVA)^2+Pgenc'*MPC.c.gencost(:,6)*(MPC.baseMVA)+sum(MPC.c.gencost(:,7))...
;
results3.VUFtotal(1,1)=V2ap(va,teta,vb,tetb,vc,tetc);
...
eval(['resultsw.w' num2str(10*w) '=results3'])
end
Results{snd,snq}=results3;
Results{snd,snq}=resultsw;
end
